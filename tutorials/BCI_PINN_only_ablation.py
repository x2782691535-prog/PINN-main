# BCI-IV-2a左右手分类 - PINN only消融实验.py
# 使用纯PINN进行左右手分类，去掉了CNN时域特征提取部分
# 实现MI-EEG信号的左右手分类
...
# 纯PINN分类模型 (去掉了CNN部分)
# =================================================================================
class PINN_Only_BCI(nn.Module):
    """
    纯PINN模型用于BCI-IV-2a左右手分类
    - 使用PINN进行源定位
    - 基于左右半球激活差异进行分类（空间感知）
    """
    def __init__(self, leadfield, fwd_model, epochs_info, in_channels=22, seq_length=1000,
                 num_classes=2, dropout_rate=0.35, sfreq=250):
        super(PINN_Only_BCI, self).__init__()
        ...
        # 获取源点的空间位置信息
        self.source_positions = self._get_source_positions(fwd_model, device)
        
        # 空间感知分类器设计
        # 1. 半球特征提取器
        self.hemisphere_analyzer = self._build_hemisphere_analyzer(device, dropout_rate)
        
        # 2. 运动皮层特征提取器  
        self.motor_cortex_analyzer = self._build_motor_cortex_analyzer(device, dropout_rate)
        
        # 3. 最终分类器
        spatial_feature_dim = 16  # 半球特征(8) + 运动皮层特征(8)
        self.classifier = nn.Sequential(
            nn.Dropout(dropout_rate),
            nn.Linear(spatial_feature_dim, 32),
            nn.ReLU(),
            nn.Dropout(dropout_rate),
            nn.Linear(32, 2)  # 二分类：左手(0) vs 右手(1)
        ).to(device)
        
        # 初始化权重
        self._initialize_weights()

    def _get_source_positions(self, fwd_model, device):
        """获取源点的空间位置信息"""
        try:
            # 尝试从前向模型获取真实源点坐标
            if 'src' in fwd_model and len(fwd_model['src']) > 0:
                # 合并左右半球的源点
                positions = []
                for src_space in fwd_model['src']:
                    if 'rr' in src_space:
                        positions.append(src_space['rr'][src_space['inuse'].astype(bool)])
                
                if positions:
                    source_positions = torch.tensor(np.vstack(positions), dtype=torch.float32).to(device)
                else:
                    source_positions = self._create_synthetic_positions(device)
            else:
                source_positions = self._create_synthetic_positions(device)
                
        except Exception as e:
            print(f"警告：无法获取真实源点位置，使用合成位置。错误：{e}")
            source_positions = self._create_synthetic_positions(device)
            
        return source_positions
    
    def _create_synthetic_positions(self, device):
        """创建合成的源点位置（当无法获取真实位置时）"""
        ...
        for i in range(n_sources):
            # 基于索引确定半球：前一半左半球(x<0)，后一半右半球(x>0)
            if i < n_sources // 2:
                # 左半球 (x < 0)
                x = -0.02 - 0.06 * torch.rand(1)  # -0.08 to -0.02
            else:
                # 右半球 (x > 0)  
                x = 0.02 + 0.06 * torch.rand(1)   # 0.02 to 0.08
                
            # y和z坐标在合理范围内随机分布
            y = -0.04 + 0.08 * torch.rand(1)  # -0.04 to 0.04
            z = 0.02 + 0.06 * torch.rand(1)   # 0.02 to 0.08
            
            positions.append([x.item(), y.item(), z.item()])
            
        return torch.tensor(positions, dtype=torch.float32).to(device)

    def _build_hemisphere_analyzer(self, device, dropout_rate):
        """构建半球激活分析器"""
        return nn.Sequential(
            nn.Linear(4, 16),  # 输入：左半球激活、右半球激活、差异、比率
            nn.ReLU(),
            nn.Dropout(dropout_rate),
            nn.Linear(16, 8),
            nn.ReLU(),
    ...
    def _build_motor_cortex_analyzer(self, device, dropout_rate):
        """构建运动皮层特征分析器"""
        return nn.Sequential(
            nn.Linear(4, 16),  # 输入：运动皮层相关特征
            nn.ReLU(), 
            nn.Dropout(dropout_rate),
            nn.Linear(16, 8),
            nn.ReLU(),
    ...
    def forward(self, x):
        ...
        source_activations = pinn_outputs['source_activations']  # (batch, n_sources)

        # 空间感知特征提取
        spatial_features = self._extract_spatial_features(source_activations)
        ...
        result = {
            ...
            'physics_loss': physics_loss,
            'hemisphere_features': spatial_features[:, :8],  # 半球特征
            'motor_features': spatial_features[:, 8:]        # 运动皮层特征
        }

        return result
    
    def _extract_spatial_features(self, source_activations):
        """
        提取空间感知特征
        Args:
            source_activations: (batch, n_sources) 源激活
        Returns:
            spatial_features: (batch, 16) 空间特征
        """
        batch_size = source_activations.shape[0]
        
        # 1. 半球激活特征提取
        hemisphere_features = self._compute_hemisphere_features(source_activations)
        
        # 2. 运动皮层特征提取
        motor_features = self._compute_motor_cortex_features(source_activations)
        
        # 通过神经网络进一步处理
        hemisphere_processed = self.hemisphere_analyzer(hemisphere_features)  # (batch, 8)
        motor_processed = self.motor_cortex_analyzer(motor_features)          # (batch, 8)
        
        # 合并特征
        spatial_features = torch.cat([hemisphere_processed, motor_processed], dim=1)  # (batch, 16)
        
        return spatial_features
    
    def _compute_hemisphere_features(self, source_activations):
        """
        计算左右半球激活特征
        Args:
            source_activations: (batch, n_sources)
        Returns:
            hemisphere_features: (batch, 4) [左半球激活, 右半球激活, 差异, 比率]
        """
        batch_size = source_activations.shape[0]
        n_sources = source_activations.shape[1]
        
        # 根据源点位置划分左右半球
        x_coords = self.source_positions[:, 0]  # x坐标
        left_mask = x_coords < 0   # 左半球 (x < 0)
        right_mask = x_coords > 0  # 右半球 (x > 0)
        
        # 计算左右半球的平均激活
        left_activation = torch.mean(source_activations[:, left_mask], dim=1)   # (batch,)
        right_activation = torch.mean(source_activations[:, right_mask], dim=1) # (batch,)
        
        # 计算激活差异和比率
        activation_diff = left_activation - right_activation
        activation_sum = left_activation + right_activation + 1e-8
        activation_ratio = activation_diff / activation_sum
        
        # 组合特征
        hemisphere_features = torch.stack([
            left_activation,
            right_activation, 
            activation_diff,
            activation_ratio
        ...
        return hemisphere_features
    
    def _compute_motor_cortex_features(self, source_activations):
        """
        计算运动皮层相关特征
        Args:
            source_activations: (batch, n_sources)
        Returns:
            motor_features: (batch, 4) 运动皮层特征
        """
        batch_size = source_activations.shape[0]
        
        # 基于源点位置识别运动皮层区域
        # 运动皮层大致位于：x: ±0.02-0.06, y: -0.02-0.02, z: 0.04-0.08
        positions = self.source_positions
        x_coords = positions[:, 0]
        y_coords = positions[:, 1] 
        z_coords = positions[:, 2]
        
        # 左侧运动皮层 (x < -0.01, y: -0.03~0.03, z: 0.03~0.08)
        left_motor_mask = (x_coords < -0.01) & (y_coords > -0.03) & (y_coords < 0.03) & (z_coords > 0.03)
        
        # 右侧运动皮层 (x > 0.01, y: -0.03~0.03, z: 0.03~0.08)  
        right_motor_mask = (x_coords > 0.01) & (y_coords > -0.03) & (y_coords < 0.03) & (z_coords > 0.03)
        
        # 计算运动皮层激活
        if left_motor_mask.sum() > 0:
            left_motor_activation = torch.mean(source_activations[:, left_motor_mask], dim=1)
        else:
            left_motor_activation = torch.zeros(batch_size, device=source_activations.device)
            
        if right_motor_mask.sum() > 0:
            right_motor_activation = torch.mean(source_activations[:, right_motor_mask], dim=1)  
        else:
            right_motor_activation = torch.zeros(batch_size, device=source_activations.device)
        
        # 运动皮层激活差异和比率
        motor_diff = left_motor_activation - right_motor_activation
        motor_sum = left_motor_activation + right_motor_activation + 1e-8
        motor_ratio = motor_diff / motor_sum
        
        # 组合运动皮层特征
        motor_features = torch.stack([
            left_motor_activation,
            right_motor_activation,
            motor_diff, 
            motor_ratio
        ], dim=1)  # (batch, 4)
...
# 训练与评估函数（纯PINN版本）
# =================================================================================
def train_model_pinn_only(data_dir, subject_id=None, num_epochs=500, batch_size=64, repeat_factor=3,
                         dropout_rate=0.35, learning_rate=0.001, physics_weight=0.05):
    """
    训练纯PINN模型函数 - 正确的交叉验证实验设计
    """
    # 记录开始时间
    ...
    for fold, (train_fold_idx, val_fold_idx) in enumerate(kf.split(train_indices)):
        ...
        for epoch in range(num_epochs):
            ...
            for batch_data, batch_labels in train_loader:
                ...
                # 总损失：主分类 + PINN物理（纯PINN版本）
                total_loss = main_class_loss + physics_weight * physics_loss
                ...
                if torch.isnan(grad_norm) or torch.isinf(grad_norm):
                    print(f"警告：梯度异常 (norm={grad_norm}), 跳过此批次")
                    optimizer.zero_grad()
                    continue
            ...
            if (epoch + 1) % 20 == 0:
                ...
                print(f"  损失分解 - 主分类: {avg_main_loss:.4f}, PINN物理: {avg_physics_loss:.6f}")
...
if __name__ == '__main__':
    # ===================== 参数控制变量（方便修改） =====================
    # 训练参数 - 优化后的设置，匹配纯PINN架构
    BATCH_SIZE = 32          # 增大批次，提高稳定性
    NUM_EPOCHS = 200         # 更多epoch配合早停
    LEARNING_RATE = 0.001    # 提高学习率，纯PINN模型可以承受
    DROPOUT_RATE = 0.4       # 高dropout防止过拟合
    PHYSICS_WEIGHT = 0.3     # PINN物理损失权重
    ...
    # ===================== 程序开始执行 =====================
    print("=== 使用纯PINN模型进行BCI-IV-2a二分类（左手/右手）- 消融实验 ===")
    print(f"训练参数配置:")
    ...
    if os.path.exists(DATA_DIR):
        ...
        for subject_id in available_subjects:
            print(f"\n=== 开始训练被试 S{subject_id} ===")
            try:
                cv_results, final_train_result, actual_epochs, final_test_result = train_model_pinn_only(
                    DATA_DIR,
        ...
        if not subject_results:
            ...
            try:
                ...
                filename = f"BCI_PINN_only_2class_results_{timestamp}_E{NUM_EPOCHS}_B{BATCH_SIZE}_R{REPEAT_FACTOR}.csv"

                df.to_csv(filename, index=False, encoding='utf-8-sig')
                print(f"\n详细结果已保存到: {filename}")

            except ImportError:
                print("\n警告: pandas未安装，跳过CSV文件保存")
            except Exception as e:
                print(f"\n警告: 保存CSV文件时出错: {e}")
