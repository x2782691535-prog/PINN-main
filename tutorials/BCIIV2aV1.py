...
# ================== 真实数据集 - 全部试次 - 预测前（真值） - EEG平均波形图 ==================
# 数据来源: 真实数据集（BCI-IV-2a），全部试次，真值（未经过模型预测）
# 图类型: EEG平均时域波形图
# 作用: 展示真实数据集所有试次的EEG平均波形
...
# ================== 真实数据集 - 全部试次 - 预测前（真值） - 头皮地形图 ==================
# 数据来源: 真实数据集（BCI-IV-2a），全部试次，真值（未经过模型预测）
# 图类型: EEG头皮电位地形图
# 作用: 展示真实数据集所有试次在不同时间点的头皮空间分布
...
# ============== 图2.1: 原始EEG头皮电位地形图 (单个时间点，用于保存) ==============
# 图类型: EEG头皮电位地形图
# 数据来源: 原始BCI-IV-2a数据 (epochs_plot.average() - 未经标准化的数据，来自第一个加载的被试，所有试次叠加平均)
# 模型/方法: 无 (原始数据特征展示)
# 目的/含义: 为之前选定的每个raw_times时间点单独生成并保存一个头皮地形图，方便单独查看。
# 所属阶段: 数据加载与探索性分析
# =======================================================================================
# 单独保存每个时间点的topomap
for t in raw_times:
...
# lambda2 = 1. / snr ** 2
# # 用Epochs对象计算协方差
# sim_epochs = simulation_test.eeg_data[idx]
...
# )
# sim_epochs_ref.set_eeg_reference(projection=True)
...
# # 应用eLORETA（Evoked输入）
# stc_elor_sim, residual_sim = mne.minimum_norm.apply_inverse(
#     sim_epochs_ref.average(), inverse_operator_sim, lambda2,
#     method=method, return_residual=True, verbose=False)
# # 可视化源空间
# brain_sim_elor = np.abs(stc_elor_sim).plot(**plot_params)
# add_text_with_chinese(brain_sim_elor, 0.1, 0.9, 'eLORETA对模拟BCI-IV-2a数据的分析', 'title', font_size=14)
# # 可视化eLORETA重建的EEG
# evoked_elor_sim = util.get_eeg_from_source(stc_elor_sim, fwd, sim_epochs_ref.info, tmin=0.)
# fig_evoked_elor_sim = evoked_elor_sim.plot()
# plt.title('eLORETA重建的模拟BCI-IV-2a信号')
# set_chinese_font_for_figure(fig_evoked_elor_sim)
# # 地形图
# valid_times_sim = [t for t in plot_times if evoked_elor_sim.times[0] <= t <= evoked_elor_sim.times[-1]]
# if len(valid_times_sim) < len(plot_times):
#     print(f"警告: 部分时间点超出eLORETA重建模拟信号范围，仅绘制有效时间点: {valid_times_sim}")
# fig_sim_topomap = evoked_elor_sim.plot_topomap(times=valid_times_sim, time_format='%0.1f s')
# fig_sim_topomap.suptitle('eLORETA重建的模拟BCI-IV-2a地形图')
# set_chinese_font_for_figure(fig_sim_topomap)

# # eLORETA对模拟数据的评价指标
# print("\n===== eLORETA对模拟数据的评价指标 (以所有测试样本为例) =====")
# method = "eLORETA"
# snr = 3.
# lambda2 = 1. / snr ** 2
...
#     sim_epochs = simulation_test.eeg_data[i]
#     sim_epochs_ref = sim_epochs.copy()
#     # 强制转换为float64并重新构造EpochsArray，确保类型兼容
#     _data = sim_epochs_ref.get_data()
#     if _data.dtype != np.float64:
...
#     true_source_stc = simulation_test.source_data[i]
