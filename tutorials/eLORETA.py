...
# 后续eLORETA分析全部基于simulation_test
...
for i in range(n_samples):
    ...
    stc_elor_sim, _ = mne.minimum_norm.apply_inverse(
    ...
    elor_y_true = true_source_stc.data[:, peak_time_idx]
    if peak_time_idx < stc_elor_sim.data.shape[1]:
        elor_y_est = stc_elor_sim.data[:, peak_time_idx]
    else:
        elor_y_est = stc_elor_sim.data[:, -1]

    elor_mle_values.append(eval_mean_localization_error(elor_y_true, elor_y_est, pos))
    elor_mse_values.append(eval_mse(elor_y_true, elor_y_est))
    auc_close, auc_far = eval_auc(elor_y_true, elor_y_est, pos)
    elor_auc_close_values.append(auc_close)
    elor_auc_far_values.append(auc_far)

print(f"eLORETA 平均定位误差 (MLE): {np.nanmean(elor_mle_values):.2f} mm")
print(f"eLORETA 均方误差 (MSE): {np.nanmean(elor_mse_values):.2e}")
all_elor_auc_values = elor_auc_close_values + elor_auc_far_values
print(f"eLORETA AUC (平均): {np.nanmean(all_elor_auc_values):.4f}")

# ========== 新增：eLORETA第一个样本的可视化（增加滤波以平滑信号） ===========
idx = 0
# 准备第一个样本用于可视化
...
stc_elor_sim.filter(l_freq=None, h_freq=40.0, fir_design='firwin', verbose=False)

# ================== eLORETA重建源空间分布多视角组合图（静态截图） ==================
# 数据来源: eLORETA重建结果，模拟测试集，样本idx
# 图类型: 3D脑源分布多视角组合图（静态）
# 作用: 展示eLORETA重建源空间分布的多视角特征
example_grid_layout = [
...
plot_stc_grid_views(
    ...
    title=f"eLORETA重建源空间分布-样本{idx} 多视角组合图",
    output_filename=os.path.join(FIG_DIR, f'eLORETA_brain_sample{idx}_composite.png')
)

# ================== eLORETA重建源空间分布峰值多视角截图（静态截图） ==================
# 数据来源: eLORETA重建结果，模拟测试集，样本idx
# 图类型: 3D脑源分布峰值多视角截图（静态）
# 作用: 展示eLORETA重建源空间分布在峰值时刻的多视角特征
peak_vertex, peak_time_idx = stc_elor_sim.get_peak(hemi=None, tmin=None, tmax=None, mode='abs', vert_as_index=True, time_as_index=True)
peak_time = stc_elor_sim.times[peak_time_idx]
print(f"eLORETA重建样本{idx}源活动峰值时间: {peak_time:.3f}s, 顶点: {peak_vertex}")
brain_elor_peak = stc_elor_sim.plot(**plot_params, initial_time=peak_time, time_viewer=True, subjects_dir=subjects_dir)
add_text_with_chinese(brain_elor_peak, 0.1, 0.9, f'eLORETA重建源-样本{idx}-峰值{peak_time:.2f}s', 'title', font_size=14)
# 不自动关闭窗口，用户可交互操作

# ================== eLORETA反投EEG波形图 ==================
# 数据来源: eLORETA重建源，反投到EEG空间，模拟测试集，样本idx
# 图类型: EEG平均波形图
# 作用: 展示eLORETA重建源反投到传感器空间后的EEG信号时域特征
evoked_elor = util.get_eeg_from_source(stc_elor_sim, fwd, eeg_data[idx].info, tmin=0.)
fig_evoked_elor = evoked_elor.plot()
plt.title('eLORETA重建的模拟BCI-IV-2a信号')
set_chinese_font_for_figure(fig_evoked_elor)

# ================== eLORETA反投EEG头皮地形图 ==================
# 数据来源: eLORETA重建源，反投到EEG空间，模拟测试集，样本idx
# 图类型: EEG头皮地形图
# 作用: 展示eLORETA重建源反投到传感器空间后的EEG信号空间分布
plot_times = [0.5, 1.0, 2.0, 3.0, 4.0]
valid_times_elor = [t for t in plot_times if evoked_elor.times[0] <= t <= evoked_elor.times[-1]]
if len(valid_times_elor) < len(plot_times):
    print(f"警告: 部分时间点超出eLORETA重建信号范围，仅绘制有效时间点: {valid_times_elor}")
fig_elor_topomap = evoked_elor.plot_topomap(times=valid_times_elor, time_format='%0.1f s')
fig_elor_topomap.suptitle('eLORETA重建的模拟BCI-IV-2a地形图')
set_chinese_font_for_figure(fig_elor_topomap)

# ================== eLORETA对真实数据集的源定位分析 ==================
print("\n===== eLORETA对真实BCI-IV-2a数据的源定位分析=====")

# 使用训练好的eLORETA方法预测真实BCI-IV-2a数据源
print("使用eLORETA方法预测真实BCI-IV-2a数据源...（左右手均分析）")
for hand_label, hand_name in zip(['left_hand', 'right_hand'], ['左手', '右手']):
    ...
    # 应用eLORETA
    stc_elor_real, _ = mne.minimum_norm.apply_inverse(
        hand_epochs_ref.average(), inverse_operator_real, lambda2,
        method=method, return_residual=True, verbose=False)

    # ================== 真实数据集 - 预测前（模型输出） - EEG平均波形图 ==================
    # 数据来源: 真实数据集（BCI-IV-2a），模型预测结果（eLORETA等）
    # 图类型: EEG平均时域波形图
    ...
    brain = stc_elor_real.plot(**plot_params)
    add_text_with_chinese(brain, 0.1, 0.9, f'eLORETA对{hand_name}BCI-IV-2a数据的预测', 'title', font_size=14)
    save_brain_screenshot(brain, f'elor_{hand_label}_real_data_brain.png')
    ...
    evoked_esi = util.get_eeg_from_source(stc_elor_real, fwd, hand_epochs.info, tmin=0.)
    fig_evoked_esi = evoked_esi.plot()
    plt.title(f'从eLORETA预测源重建的{hand_name}BCI-IV-2a信号')
    ...
    fig.suptitle(f'eLORETA预测{hand_name}BCI-IV-2a地形图')
    set_chinese_font_for_figure(fig)

    # ============== 多视角组合图演示 ==============
    if stc_elor_real is not None:
        print(f"正在使用 eLORETA 模型预测的{hand_name}数据 stc 生成组合视图...")
        example_grid_layout = [
            [ {'view': 'lateral', 'hemi': 'lh'}, {'view': 'dorsal', 'hemi': 'both'}, {'view': 'lateral', 'hemi': 'rh'} ],
            [ {'view': 'medial', 'hemi': 'lh'}, {'view': 'ventral', 'hemi': 'both'}, {'view': 'medial', 'hemi': 'rh'} ]
        ]
        ...
        plot_stc_grid_views(
            ...
            base_plot_params=custom_base_params,
            figsize_per_subplot=(3.5, 3.5),
            title=f"eLORETA对{hand_name}数据的多视角组合图",
            output_filename=os.path.join(FIG_DIR, f'elor_{hand_label}_real_data_composite_views.png')
        )
    else:
