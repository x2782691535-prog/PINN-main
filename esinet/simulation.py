...
class Simulation:
    ...
    def generate_balanced_sources(self, number_of_sources):
        """确保两个源点分布在左右半球（严格基于src结构）"""
        lh_verts = self.fwd['src'][0]['vertno']
        rh_verts = self.fwd['src'][1]['vertno']
        if number_of_sources == 2:
            left_idx = np.random.choice(lh_verts, 1)[0]
            right_idx = np.random.choice(rh_verts, 1)[0]
            return np.array([left_idx, right_idx])
        else:
            all_verts = np.concatenate([lh_verts, rh_verts])
            return np.random.choice(all_verts, number_of_sources, replace=False)
    ...
    def simulate_source(self):
        ...
        src_centers = self.generate_balanced_sources(number_of_sources)
        duration_of_trial = self.get_from_range(
            self.settings['duration_of_trial'], dtype=float
        )
