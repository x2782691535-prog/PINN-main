...
class Net:
    ...
    def __init__(self, fwd, n_dense_layers=1, n_lstm_layers=2, 
        ...
        scale_individually=True, rescale_sources='brent', 
                 ...
                 dirichlet_weight_initial=0.4, neumann_weight_initial=0.3,
                 reference_weight_initial=0.2, robin_weight_initial=0.1):
        ...
        self.neumann_weight_initial = neumann_weight_initial
        self.reference_weight_initial = reference_weight_initial
        ...
        if not self.compiled:
            ...
            if self.model_type.lower() == 'pinn' and self.use_pinn:
                ...
                def custom_physics_loss(y_true, y_pred):
                    ...
                    # 完整的边界条件损失（包含狄利克雷、诺伊曼、参考电极、Robin）
    ...
    def boundary_condition_loss(self, source_activations):
        """
        真正的边界条件损失：实现狄利克雷、诺伊曼、参考电极、Robin边界条件
        ...
        # 2. 诺伊曼边界条件：∂φ/∂n = 0 (法向导数为零)
        neumann_loss = self._neumann_boundary_loss(source_activations, n_sources)
        
        # 3. 参考电极约束：φ(参考电极) = 0
        reference_loss = self._reference_electrode_loss(source_activations)
        
        # 4. Robin边界条件：∂φ/∂n + γφ = h (混合边界条件)
        robin_loss = self._robin_boundary_loss(source_activations, n_sources)
        
        # 使用可训练权重组合所有边界条件损失
        (dirichlet_w, neumann_w, reference_w, robin_w) = self.get_normalized_boundary_weights()
        
        total_boundary_loss = (
            dirichlet_w * dirichlet_loss +     # 狄利克雷边界条件
            neumann_w * neumann_loss +         # 诺伊曼边界条件
            reference_w * reference_loss +     # 参考电极约束
    ...
    def _dirichlet_boundary_loss(self, source_activations, n_sources):
        ...
        # 通过leadfield计算电势分布
        with tf.device('/GPU:0'):
            leadfield_tensor = tf.constant(self.leadfield, dtype=tf.float32)  # (n_sources, n_channels)
            # 正确的矩阵乘法：(batch, n_sources, n_time) × (n_sources, n_channels) -> (batch, n_channels, n_time)
            potentials = tf.einsum('bst,sc->bct', source_activations, leadfield_tensor)
        
        # 定义边界电极（例如，最外围的电极作为边界）
        n_channels = tf.shape(potentials)[1]
        boundary_electrode_indices = self._get_boundary_electrode_indices(n_channels)
        
        boundary_potentials = tf.gather(potentials, boundary_electrode_indices, axis=1)
        
        # 狄利克雷边界条件：边界电势应等于指定值（这里设为0）
        target_boundary_potential = 0.0
        dirichlet_loss = tf.reduce_mean(tf.square(boundary_potentials - target_boundary_potential))
    ...
    def _neumann_boundary_loss(self, source_activations, n_sources):
        ...
        # 获取真实的电极坐标
        # 从前向模型中提取电极位置信息
        if hasattr(self.fwd, 'info') and 'chs' in self.fwd['info']:
            # 使用真实的电极坐标
            electrode_positions = []
            for ch in self.fwd['info']['chs']:
                if 'eeg' in ch['ch_name'].lower():
                    # EEG电极位置通常存储在loc字段的前3个元素中
                    pos = ch['loc'][:3]
                    electrode_positions.append(pos)
            
            if len(electrode_positions) >= n_channels:
                electrode_coords = tf.constant(electrode_positions[:n_channels], dtype=tf.float32)
            else:
                # 如果电极位置信息不足，使用标准10-20系统的位置
                electrode_coords = self._get_standard_electrode_positions(n_channels)
        else:
            # 使用标准10-20系统的电极位置
            electrode_coords = self._get_standard_electrode_positions(n_channels)
        
        x_electrodes = electrode_coords[:, 0]
        y_electrodes = electrode_coords[:, 1]
        z_electrodes = electrode_coords[:, 2]
        
        # 计算真实的边界法向量
        # 假设头部为球形，法向量指向球心外侧
        center = tf.reduce_mean(electrode_coords, axis=0)  # 头部中心
        boundary_coords = tf.gather(electrode_coords, boundary_electrode_indices)
        
        # 计算从中心指向边界点的向量
        normal_vectors = boundary_coords - center[None, :]
        # 归一化法向量
        normal_magnitudes = tf.norm(normal_vectors, axis=1, keepdims=True)
        normal_magnitudes = tf.maximum(normal_magnitudes, 1e-8)
        normalized_normals = normal_vectors / normal_magnitudes
        ...
        for t in range(n_timepoints):
            ...
            for b in range(batch_size):
                ...
                # 创建可微分的电极坐标变量
                x_coords = tf.Variable(x_electrodes, trainable=False, dtype=tf.float32)
                y_coords = tf.Variable(y_electrodes, trainable=False, dtype=tf.float32)
                z_coords = tf.Variable(z_electrodes, trainable=False, dtype=tf.float32)
                
                # 使用球面调和插值构建连续电势场
                def spherical_interpolation(x, y, z, potentials, electrode_coords):
                    """球面调和插值"""
                    query_points = tf.stack([x, y, z], axis=1)
                    
                    # 计算到所有电极的距离
                    distances = tf.norm(query_points[:, None, :] - electrode_coords[None, :, :], axis=2)
                    
                    # 使用反距离加权插值
                    weights = 1.0 / (distances + 1e-8)
                    weights = weights / tf.reduce_sum(weights, axis=1, keepdims=True)
                    
                    interpolated = tf.reduce_sum(weights * potentials[None, :], axis=1)
                    return interpolated
                
                # 使用GradientTape计算电势的空间梯度
                with tf.GradientTape() as tape:
                    tape.watch([x_coords, y_coords, z_coords])
                    
                    # 使用球面插值构建连续电势场
                    phi_continuous = spherical_interpolation(x_coords, y_coords, z_coords, 
                                                           sample_potentials, electrode_coords)
                
                # 计算电势梯度
                dphi_dx = tape.gradient(phi_continuous, x_coords)
                dphi_dy = tape.gradient(phi_continuous, y_coords)
                dphi_dz = tape.gradient(phi_continuous, z_coords)
                
                # 处理None梯度
                if dphi_dx is None:
                    dphi_dx = tf.zeros_like(x_coords)
                if dphi_dy is None:
                    dphi_dy = tf.zeros_like(y_coords)
                if dphi_dz is None:
                    dphi_dz = tf.zeros_like(z_coords)
                
                # 获取边界处的电势梯度
                boundary_dphi_dx = tf.gather(dphi_dx, boundary_electrode_indices)
                boundary_dphi_dy = tf.gather(dphi_dy, boundary_electrode_indices)
                boundary_dphi_dz = tf.gather(dphi_dz, boundary_electrode_indices)
                
                # 计算法向导数: ∂φ/∂n = ∇φ · n
                normal_derivatives = (boundary_dphi_dx * normalized_normals[:, 0] + 
                                    boundary_dphi_dy * normalized_normals[:, 1] + 
                                    boundary_dphi_dz * normalized_normals[:, 2])
                
                # 诺伊曼边界条件：∂φ/∂n = 0
                sample_loss = tf.reduce_mean(tf.square(normal_derivatives))
                batch_losses.append(sample_loss)
            
            # 合并批次损失
            time_loss = tf.reduce_mean(tf.stack(batch_losses))
            neumann_losses.append(time_loss)
        
        # 返回所有时间点的平均损失
        return tf.reduce_mean(tf.stack(neumann_losses))
    ...
    def _reference_electrode_loss(self, source_activations):
        ...
        # 选择参考电极（通常是Cz或平均参考）
        n_channels = tf.shape(potentials)[1]
        reference_electrode_idx = n_channels // 2  # 选择中央电极作为参考
        
        reference_potential = potentials[:, reference_electrode_idx, :]  # (batch, time)
        
        # 参考电极约束：参考电极电势应为0
        reference_loss = tf.reduce_mean(tf.square(reference_potential))
    ...
    def _robin_boundary_loss(self, source_activations, n_sources):
        ...
        # 计算真实的边界法向量
        center = tf.reduce_mean(electrode_coords, axis=0)
        boundary_coords = tf.gather(electrode_coords, boundary_electrode_indices)
        normal_vectors = boundary_coords - center[None, :]
        normal_magnitudes = tf.norm(normal_vectors, axis=1, keepdims=True)
        normal_magnitudes = tf.maximum(normal_magnitudes, 1e-8)
        ...
        # Robin边界条件参数（基于物理特性，不是简化）
        # γ与组织的电导率和几何特性相关
        tissue_conductivity = 0.33  # 大脑组织电导率 (S/m)
        boundary_thickness = 0.003  # 边界层厚度约3mm
        gamma = tissue_conductivity / boundary_thickness  # 物理上合理的混合系数
        h = 0.0  # 在无外部电流源的情况下，右侧项为0
        ...
        for t in range(n_timepoints):
            ...
            for b in range(batch_size):
                ...
                # Robin边界条件：∂φ/∂n + γφ = h
                # 重新排列：∂φ/∂n + γφ - h = 0
                robin_residual = normal_derivatives + gamma * boundary_potentials - h
                
                # 计算损失（残差的平方）
                sample_loss = tf.reduce_mean(tf.square(robin_residual))
                batch_losses.append(sample_loss)
    ...
    def _create_trainable_weights(self):
        ...
        # 诺伊曼边界条件权重（可训练）
        self.trainable_neumann_weight = tf.Variable(
            initial_value=self.neumann_weight_initial,
            trainable=True,
            dtype=tf.float32,
            name='neumann_weight'
        )
        ...
        # 参考电极约束权重（可训练）
        self.trainable_reference_weight = tf.Variable(
            initial_value=self.reference_weight_initial,
            trainable=True,
            dtype=tf.float32,
    ...
    def get_normalized_boundary_weights(self):
        ...
        -------
        tuple
            (dirichlet_weight, neumann_weight, reference_weight, robin_weight)
        """
        # 对边界条件内部权重进行归一化
        boundary_weights = tf.nn.softmax([
            self.trainable_dirichlet_weight,
            self.trainable_neumann_weight,
            self.trainable_reference_weight,
            self.trainable_robin_weight
        ])
        
        return (
            boundary_weights[0],  # dirichlet_weight
            boundary_weights[1],  # neumann_weight
            boundary_weights[2],  # reference_weight
            boundary_weights[3]   # robin_weight
        )
    ...
    def get_all_physics_weights(self):
        ...
        return {
            ...
            'neumann_weight': float(neumann_w),
            'reference_weight': float(reference_w),
    ...
    def print_current_weights(self):
        ...
        print(f"     ├─ 狄利克雷边界: {weights['dirichlet_weight']:.4f}")
        print(f"     ├─ 诺伊曼边界: {weights['neumann_weight']:.4f}")
        print(f"     ├─ 参考电极: {weights['reference_weight']:.4f}")
