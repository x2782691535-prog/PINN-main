...
def train(dataset_conf, train_conf, results_path):
    ...
    for sub in range(n_sub): # (num_sub): for all subjects, (i-1,i): for the ith subject.
        ...
        for train in range(n_train): # How many repetitions of training for subject i.
            ...
            # Use categorical crossentropy even for binary classification to maintain consistency
            loss_function = CategoricalCrossentropy(from_logits=from_logits)
            metrics = ['accuracy']
            
            # Compile and train the model
            model.compile(loss=loss_function, optimizer=Adam(learning_rate=lr), metrics=metrics)          
    ...
    # Print average performance for all subjects
    info = '\nAverage Performance: Accuracy = {:.4f}, Kappa = {:.4f}\n'.format(np.average(acc), np.average(kappa))
    print(info)
    log_write.write(info)
...
def run():
    ...
    if dataset == 'BCI2a': 
        ...
        classes_labels = ['Left hand', 'Right hand']  # Only two classes
        # data_path = os.path.expanduser('~') + '/BCI Competition IV/BCI Competition IV-2a/BCI Competition IV 2a mat/'
        data_path = 'E:/pycharm/EEG-ATCNet/BCI2a/'
    elif dataset == 'HGD':
        in_samples = 1125
        n_channels = 44
        n_sub = 14
        n_classes = 2  # Binary classification
        classes_labels = ['Right Hand', 'Left Hand']  # Only two classes     
        ...
        n_sub = 18
        n_classes = 2  # Binary classification
        classes_labels = ['Fingers', 'Wrist']  # Only two classes     
    ...
    results_path = os.getcwd() + "/results_binary"
    ...
    train_conf = { 'batch_size': 64, 'epochs': 1000, 'patience': 300, 'lr': 0.001,
                  'LearnCurves': True, 'n_train': 10, 'model':'ATCNet', 'from_logits': False}
           
    # Train the model
    train(dataset_conf, train_conf, results_path)

    # Evaluate the model based on the weights saved in the '/results_binary' folder
