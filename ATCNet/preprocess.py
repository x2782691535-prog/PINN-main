...
def load_data_LOSO (data_path, subject, dataset): 
    ...
    for sub in range(0, 9):
        ...
        if dataset == 'BCI2a':
            X1, y1 = load_BCI2a_data(path, sub+1, True, binary_classes=[1, 2])
            X2, y2 = load_BCI2a_data(path, sub+1, False, binary_classes=[1, 2])
        elif dataset == 'CS2R':
...
#%%
def load_BCI2a_data(data_path, subject, training, all_trials = True, binary_classes=[1, 2]):
    ...
    and 288 x 9 trials in session 2 for testing.  
        ...
        binary_classes: list
            list of class indices to keep for binary classification (1=Left hand, 2=Right hand, 3=Foot, 4=Tongue)
    """
    ...
    if training:
        ...
        # a = sio.loadmat(data_path + 'processed_A0' + str(subject) + 'T.mat')
    else:
    ...
    for ii in range(0, a_data.size):
        ...
        for trial in range(0, a_trial.size):
             if(a_artifacts[trial] != 0 and not all_trials):
                 continue
             # Check if this trial belongs to one of the selected binary classes
             if int(a_y[trial]) in binary_classes:
    ...
    # Remap class labels for binary classification (0 and 1)
    if len(binary_classes) == 2:
        # Map first class to 0, second class to 1
        class_mapping = {binary_classes[0]-1: 0, binary_classes[1]-1: 1}
        class_return = np.array([class_mapping[label] for label in class_return])
...
def get_data(path, subject, dataset='BCI2a', classes_labels='all', LOSO = False, isStandard = True, isShuffle = True):
    ...
    if LOSO:
        ...
        if dataset == 'BCI2a':
            ...
            X_train, y_train = load_BCI2a_data(path, subject+1, True, binary_classes=[1, 2])
            X_test, y_test = load_BCI2a_data(path, subject+1, False, binary_classes=[1, 2])
        elif dataset == 'CS2R':
